...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // 验证上传的文件
        validateUploadedFile($_FILES['file']);

        // 生成文件名
        $timestamp = time();
        $originalFileName = $timestamp . '_original_' . basename($_FILES['file']['name']);
        $processedFileName = $timestamp . '_processed_' . basename($_FILES['file']['name']);
